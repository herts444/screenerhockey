"""
Data service for Austrian ICE Hockey League.
"""
...
from .austria_api import AustriaApiService, AUSTRIA_TEAM_NAMES_RU
from .stats_calculator import StatsCalculator, GameResult


class AustriaDataService:
    """Service for managing Austrian ICE HL data and calculating statistics"""

    LEAGUE = "AUSTRIA"

    def __init__(self):
        self.api = AustriaApiService()

    async def sync_teams(self, db: Session) -> List[Team]:
        """Sync all ICE HL teams to database"""
        ...
        for team_data in teams_data:
            ...
            if existing:
                ...
                existing.name_ru = AUSTRIA_TEAM_NAMES_RU.get(abbrev)
                ...
                team = Team(
                    ...
                    abbrev=abbrev,
                    name=team_data["name"],
                    name_ru=AUSTRIA_TEAM_NAMES_RU.get(abbrev),
        ...
        # Log update
        update = DataUpdate(update_type="austria_teams_sync")
        db.add(update)
        db.commit()
    ...
    async def sync_team_games(self, db: Session, team_id: str) -> int:
        ...
        for game_data in games_data:
            ...
            is_finished = game_data.get("status") == "AFTER_MATCH"
    ...
    async def sync_all_games(self, db: Session) -> int:
        """Sync games for all ICE HL teams"""
        ...
        for team in teams:
            try:
                ...
                print(f"Error syncing Austria team {team.abbrev}: {e}")

        # Log update
        update = DataUpdate(update_type="austria_games_sync")
        db.add(update)
        db.commit()
    ...
    def get_team_stats(self, db: Session, team_abbrev: str, last_n: int = 0) -> dict:
        """Get complete stats for an ICE HL team"""
    ...
    async def get_upcoming_games(self, db: Session, days: int = 7) -> List[dict]:
        """Get upcoming ICE HL games for the next N days"""
        ...
        for game in games:
            ...
            result.append({
                "game_id": f"austria_{game_id}",
                "date": game_date.strftime("%d.%m.%Y") if game_date else "",
                "date_iso": game_date.isoformat() if game_date else "",
                "home_team": {
                    "abbrev": home_abbrev,
                    "name": home_data.get("name", home_abbrev),
                    "name_ru": home_team.name_ru if home_team else AUSTRIA_TEAM_NAMES_RU.get(home_abbrev),
                    "logo_url": home_team.logo_url if home_team else None
                },
                "away_team": {
                    "abbrev": away_abbrev,
                    "name": away_data.get("name", away_abbrev),
                    "name_ru": away_team.name_ru if away_team else AUSTRIA_TEAM_NAMES_RU.get(away_abbrev),
    ...
    def get_match_analysis(
        ...
        """Get complete analysis for an upcoming ICE HL match"""
